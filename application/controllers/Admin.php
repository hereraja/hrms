<?php
	...
	public function employee() {		//Employee Dashboard
		...
		$select = array("a.emp_code", "a.emp_name", "a.emp_catg",
						"a.department",'b.district_name','c.name as dept_name');

		$employee['employee_dtls']    =   $this->Admin_Process->f_get_particulars("md_employee a,md_district b,md_department c", $select, array("a.emp_dist = b.district_code"=> NULL,"a.department = c.id"=> NULL, "a.emp_status" => 'A'), 0);
		//Category List 
    ...
    {       // $pass  = 123;
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
			...
			$employee_dt   =  $this->Admin_Process->f_get_particulars("md_employee",NULL, array("emp_code" => $emp_id),1);
               ...
               $pass  = 123;
                $data_array = array(
                    ...
                    "user_name"     =>  $employee_dt->emp_name,
                    "user_status"     =>  'A',
				...
				$query = $this->db->get_where('md_users', array('user_id' => trim($this->input->post('user_id'))));
			...
			$data['employee_dtls']   =  $this->Admin_Process->f_get_particulars("md_employee",NULL, array("emp_status" => 'A','salary_status'=>1), 0);
	...
	public function wagemp_employee() {		//Employee Dashboard

		//Employee List
		$select = array("a.emp_code", "a.emp_name", "a.emp_catg",
						"a.department",'b.district_name','c.name as dept_name');

		$employee['employee_dtls']  =   $this->Admin_Process->f_get_particulars("md_employee a,md_district b,md_department c", $select, array("a.emp_dist = b.district_code"=> NULL,"a.department = c.id"=> NULL, "a.emp_status" => 'A', "a.emp_catg" => 4), 0);
		//Category List 
		$employee['category_dtls']  =   $this->Admin_Process->f_get_particulars("md_category", NULL, NULL, 0);

		$this->load->view('post_login/payroll_main');
		$this->load->view("employee/dashboard_wagemp", $employee);
		$this->load->view('post_login/footer');
		
	}

	public function wageemployee_edit(){		//Edit Employee

		if($_SERVER['REQUEST_METHOD'] == "POST") {

		$this->form_validation->set_rules('no_of_days', 'Basic Pay', 'required');
		if($this->form_validation->run() == TRUE){
			...
			$select = array ("emp_code", "emp_name", "emp_catg","emp_dist", "dob","email", "phn_no",
							 "designation", "department","emp_addr","salary_status",
							 "pan_no", "bank_name", "bank_ac_no", "ifsc","join_dt","ret_dt","no_of_days",
							 "pf_ac_no","uan","basic_pay","aadhar_no","emp_status");
			...
			$employee['dept']          =   $this->Admin_Process->f_get_particulars("md_department", NULL, NULL, 0);
			$this->load->view('post_login/payroll_main');
			$this->load->view("employee/edit_wagemp", $employee);
			$this->load->view('post_login/footer');
		}
	}
