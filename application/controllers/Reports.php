    ...
    if($_SERVER['REQUEST_METHOD'] == "POST") {
       ...
       // echo $this->db->last_query();die();
        ...
        if($this->input->post('category') == 4){
            $this->load->view("reports/salary_wages", $salary);
        }else{
        ...
        //Category List
        $salary['category']   =   $this->Report_Process->f_get_particulars("md_category", NULL, array('category_code IN (1,2,3,4)' => NULL), 0);
    ...
    public function paystatementreport(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $select = array(
                ...
                "(t.basic_pay+t.grade_pay+t.da_amt+t.hra_amt+t.med_allow+t.othr_allow) - (t.insuarance+t.ccs+t.hbl+t.telephone+t.med_adv+t.festival_adv+t.tf+t.med_ins+t.comp_loan+ t.ptax + t.itax + t.gpf +t.epf + t.other_deduction) as net_amount"
            );
            ...
            //Category List
            $statement['category']   =   $this->Report_Process->f_get_particulars("md_category", NULL, array('category_code IN (1,2,3,4)' => NULL), 0);
 ...
 public function totaldeduction () {
    ...
    public function payslipreport() {
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $payslip['emp_list']   =   $this->Report_Process->f_get_particulars("md_employee", $select, array("salary_status" => 1 ,'1 order by emp_name'=>NULL), 0);
    ...
    public function indpayslip() {

        if($_SERVER['REQUEST_METHOD'] == "POST") {

            //Payslip
            //$empno     =  $this->session->userdata['loggedin']['emp_cd'];
            $empno = 27;
            $sal_month  = $this->input->post('sal_month');
            $sal_yr     = $this->input->post('year');
            ...
            $payslip['emp_dtls']    =   $this->Report_Process->f_get_particulars("md_employee", NULL, array("emp_code" =>  $empno), 1);
            ...
            $this->load->view('post_login/payroll_main');
            $this->load->view("reports/indpayslip", $payslip);
            $this->load->view('post_login/footer');
    ...
    public function pfledger() {
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $empno     =  $this->input->post('emp_cd');
           // $sal_month  = $this->input->post('sal_month');
            $ssal_yr     = $this->input->post('syear');
            $esal_yr     = $this->input->post('eyear');
    ...
    public function pfledger_indivisual() {
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $this->load->view('post_login/payroll_main');
            $this->load->view("reports/pfledger_indivisual", $payslip);
            $this->load->view('post_login/footer');
